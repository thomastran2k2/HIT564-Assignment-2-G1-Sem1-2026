...
# ── Additional EDA: ACF plot ──────────────────────────────────────────────────
# Justifies lag features used in regression
# ACF shows correlation between assault_rate(t) and assault_rate(t-lag)
# Significant bars beyond confidence band -> temporal autocorrelation exists

from statsmodels.tsa.stattools import acf as compute_acf

print()
print("-- Additional EDA: ACF plot (justifying lag features) --")

# Aggregate assault to region x month for ACF analysis
assault_acf_df = (
    assault_df.groupby(["Year", "Month number", "Region"])
...
assault_acf_df["Assault_rate_100k"] = (
    assault_acf_df["Assault_offences"] / assault_acf_df["Total_population"] * 100000
).round(2)

REGION_ORDER_ACF = ["Greater Darwin", "Central Australia", "Big Rivers",
                    "East Arnhem", "Barkly", "Top End"]
REGION_PALETTE_ACF = sns.color_palette("tab10", n_colors=6)

fig, axes = plt.subplots(2, 3, figsize=(14, 8))
axes = axes.flatten()
n_obs = 24  # 24 months per region
ci_bound = 1.96 / np.sqrt(n_obs)  # 95% confidence interval

for idx, (region, color) in enumerate(zip(REGION_ORDER_ACF, REGION_PALETTE_ACF)):
    ax = axes[idx]
    series = assault_acf_df[assault_acf_df["Region"] == region]["Assault_rate_100k"].values
    acf_vals = compute_acf(series, nlags=6, fft=False)

    lags = range(1, 7)
    bar_colors = ["#EF5350" if abs(v) > ci_bound else "#90CAF9" for v in acf_vals[1:]]
    ax.bar(lags, acf_vals[1:], color=bar_colors, edgecolor="white", width=0.6)
    ax.axhline(ci_bound,  color="black", linewidth=1.2, linestyle="--", alpha=0.6,
               label=f"95% CI (±{ci_bound:.2f})")
    ax.axhline(-ci_bound, color="black", linewidth=1.2, linestyle="--", alpha=0.6)
    ...
    if idx == 0:
        ax.legend(fontsize=8)
    # Annotate lag1 value
    ax.text(1, acf_vals[1] + 0.05, f"{acf_vals[1]:.2f}",
            ha="center", fontsize=9, fontweight="bold",
            color="#EF5350" if abs(acf_vals[1]) > ci_bound else "#1565C0")

fig.suptitle(
    "Autocorrelation Function (ACF) of Monthly Assault Rate per 100k by Region\n"
    "(Red bars exceed 95% confidence band — significant autocorrelation)",
    fontsize=12, fontweight="bold"
)
fig.tight_layout()
save(fig, "3_7_acf_assault_by_region.png")

# Print summary
print("  ACF lag1 by region (95% CI = ±{:.3f}):".format(ci_bound))
for region in REGION_ORDER_ACF:
    series = assault_acf_df[assault_acf_df["Region"] == region]["Assault_rate_100k"].values
    acf_vals = compute_acf(series, nlags=3, fft=False)
    sig = "SIGNIFICANT" if abs(acf_vals[1]) > ci_bound else "not significant"
    print(f"    {region:<20} lag1={acf_vals[1]:.3f} lag3={acf_vals[3]:.3f}  [{sig}]")
